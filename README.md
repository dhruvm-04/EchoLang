...
## Rebuild Status (March 2026)

This project has been rebuilt to support a reproducible deployment flow:

1. Train/build in Kaggle.
2. Export a single `echolang_bundle.pkl` artifact.
3. Load that artifact in Streamlit for public demo hosting.

The rebuilt code lives in `src/echolang/`.

## What Was Fixed

- Added a modular package structure (`src/echolang`) instead of notebook-only logic.
- Added a deterministic build/export script: `kaggle_build_artifact.py`.
- Added a Streamlit runtime loader: `streamlit_app.py`.
- Added an issue audit report: `ISSUES_FOUND.md`.
- Added deployment guide: `KAGGLE_TO_STREAMLIT.md`.

## Quickstart

### Build artifact (Kaggle)
...
```text
src/echolang/pipeline.py   # Training, inference, PKL save/load
src/echolang/cli.py        # CLI inference helper
kaggle_build_artifact.py   # Kaggle export entrypoint
streamlit_app.py           # Streamlit UI using PKL artifact
```

## Legacy Notebook

`EchoLang.ipynb` is kept for reference, but it is not the recommended deployment path.
